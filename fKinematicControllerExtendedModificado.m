...
wr= vw;
vr= vx*cos(p3dx.pPos.X(6)) + vy*sin(p3dx.pPos.X(6)) + p3dx.pPar.a*sin(p3dx.pPar.alpha)*vw;

% Control law

p3dx.pSC.Ud(2) = wr+pgains(2)*vr*p3dx.pPos.Xtil(2)+pgains(3)*vr*sin(p3dx.pPos.Xtil(3));
p3dx.pSC.Ud(1) = vr*cos(p3dx.pPos.Xtil(3))+pgains(1)*p3dx.pPos.Xtil(1);
